...
parser.add_option("-v", "--version", dest="version", default='1.4.3',
                          help='Use a specific zc.buildout version.  *This '
                          'bootstrap script defaults to '
                          '1.4.3, unlike usual buildpout bootstrap scripts.*')
...
try:
    ...
    ez = {}
    if USE_DISTRIBUTE:
        exec urllib2.urlopen('https://python-distribute.org/distribute_setup.py'
                         ).read() in ez
        ez['use_setuptools'](to_dir=tmpeggs, download_delay=0, no_fake=True)
    else:
        exec open('/app/ez_setup.py'
                             ).read() in ez
